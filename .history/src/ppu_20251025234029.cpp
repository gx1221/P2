...
void PPU::render() {
  ...
  if (!bgEnabled || (inLeft8 && !bgLeft)) background_color_index = 0;

  // --- Background palette lookup & draw ---
  uint8_t  bg_pal_byte = (background_color_index == 0)
                      ? palette_RAM[0]
                      : palette_RAM[((palette_high_bits << 2) | (background_color_index & 0x03)) & 0x1F];
