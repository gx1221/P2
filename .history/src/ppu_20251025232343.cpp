...
const bool sprLeft    = (mask & 0x04) != 0; // show sprites in leftmost 8 px

const bool visibleDot = (scanline < 240) && (ppu_cycles >= 1 && ppu_cycles <= 256);
const int  xdot       = ppu_cycles - 1;

const bool bgEnabled  = (mask & 0x08) != 0; // show background
const bool bgLeft     = (mask & 0x02) != 0; // show background in leftmost 8 px
...
// --- Fine X handling within current 8-dot group ---
uint8_t coarseX_base = (vram_addr & 0x1F);
uint8_t groupDot     = (xdot & 7);
uint8_t fxSum        = (this->x + groupDot);   // 0..15
uint8_t pxInTile     = (fxSum & 7);            // pixel within tile (0..7)
uint8_t carryTile    = (fxSum >> 3);           // crossed into next tile? (0/1)

uint8_t localCoarseX = ((coarseX_base + carryTile) & 31);
uint8_t pageX        = (((coarseX_base + carryTile) >> 5) & 1);
uint8_t ntX          = (ntXBit ^ pageX);

// --- Name table base & tile index (with carry applied) ---
...
// --- Pattern fetch & pixel select for background ---
