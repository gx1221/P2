...
PPU::PPU() {
  ...
  // make sure everything doesn't have some garbage values
...
void PPU::write_register(uint16_t cpu_addr, uint8_t value) {
  ...
  if (!write_latch) {
    ...
    // Copy v  if we're not in the rendering fetch windows.
...
uint8_t PPU::read_register(uint16_t cpu_addr) {
    ...
    switch (cpu_addr % 8) {
      ...
      if (addr < 0x3F00) {
        ...
        // palette direct read  and refill buffer from addr - $1000
        uint16_t pal = (addr - 0x3F00) & 0x1F;
        if ((pal & 0x13) == 0x10) pal &= ~0x10;
        out = palette_RAM[pal];
        uint16_t nt = (addr - 0x1000) & 0x2FFF;
        buffer = mapper->read_ppu(nt);
      }
